window.onload = function() {
  var 
      ...
      // queue()
      //   .defer(d3.tsv, rootUrl + scenario + "/" + algorithm + "/communities.csv") // topojson polygons
      //   // .defer(d3.tsv, 'cities.json') // geojson points
      //   .await(gotAllData); // function that uses files
      ...
      function gotAllData(error, vehicles) {
        gotVehicles(vehicles);
      }
      ...
      function gotVehicles(data) {
        ...
        function redraw(time) {
          nest.filter(function(step) {return step.key == time}).forEach(function(timeStep) {
            var vertices = timeStep.values
            voronoi(vertices).forEach(function(d,i) {
              if (d.length > 0) {
                ctx.fillStyle = color(vertices[i].com_id);
                ctx.strokeStyle = color(vertices[i].com_id);
                ctx.beginPath();
      ...
      function updateSpeedScale(metric) {
        ...
        for (var i = firstStep; i < lastStep; i += stepSize)  {
           ...
           minYStep = d3.min(vehicles[i], function(d){ return d[metric] })
           if (minYStep < minY) {
            minY = minYStep;
           }
        }
        // console.log("min and max speed ever", metric, minY, maxY)
